...
| pay | `<amount>` | The Item to pay is determined by the item held when the command is run to set this action. Allows players to pay into the sign to unlock subsequent actions. Example: `/easy-sign pay 32` while holding a diamond would require a player to pay 32 diamonds before the next action runs. Once paid off, no more payment is needed and subsequent actions run every time. |
